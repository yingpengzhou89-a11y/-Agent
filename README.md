...
首次生成报告时，Progress Service 会使用可解释的 `review_schedule/v2` 更新每项技能的掌握度与复习时间：掌握度低于 60 分次日复习、60—79 分三天后复习、80 分及以上七天后复习；连续两次及以上高分会逐步延长至最多 21 天，连续两次低分会强制缩短至次日。系统同时保存连续答对/答错次数、最近得分与规则版本；重复生成同一报告不会重复累计训练次数。
...
知识库默认以约 900 字符、120 字符重叠切分，并优先按标题、段落和句子断开，以避免单个命中片段夹带整篇无关内容。已有资料可在页面点击“重新切分并重建全部索引”升级为新切分策略；该操作会保留原始文件，只替换数据库中的检索片段和向量。

检索会先做 FTS、字符 n-gram 与向量候选召回，再按向量相似度、词法匹配和 RRF 名次进行轻量重排序；默认只展示 Top 3，且同一文档最多保留 2 个片段，低置信候选会被过滤。

用户可对引用标记“有帮助”或“不相关”。这些反馈在当前用户范围内以受限权重参与后续排序：单次反馈仅产生很小的加分或降权，连续三次同方向反馈才达到最大影响，避免偶然点击主导检索结果。

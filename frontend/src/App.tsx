...
import type { DocumentRecord, Evaluation, Match, Plan, Session, User } from './types'
...
function InterviewPanel({ userId, ids, saveIds, show, requireUser }: PanelProps) {
  const [plan, setPlan] = useState<Plan | null>(null); const [session, setSession] = useState<Session | null>(null); const [question, setQuestion] = useState(''); const [answer, setAnswer] = useState(''); const [evaluation, setEvaluation] = useState<Evaluation | null>(null); const [reportData, setReportData] = useState<{ summary: { overall_score: number; evaluated_question_count: number; strong_skills: string[]; weak_topics: string[]; dimension_scores: Record<string, number> }; weak_topics: string[]; recommended_actions: string[] } | null>(null)
  const pendingEvaluation = session?.status === 'ANSWER_SAVED' || session?.status === 'EVALUATING'
  const createPlan = async () => { const p = await api<Plan>('/api/v1/interview-plans', { method: 'POST', body: JSON.stringify({ resume_id: ids.resumeId, job_id: ids.jobId }) }, requireUser()); setPlan(p); saveIds({ ...ids, planId: p.id }); show('面试计划已生成') }
  const loadQuestion = async (sessionId = session?.id) => { if (!sessionId) return; const q = await api<{ question_text: string }>(`/api/v1/interviews/${sessionId}/question`, {}, requireUser()); setQuestion(q.question_text); setAnswer(''); setEvaluation(null) }
  ...
  const start = async () => { if (session) { if (session.status === 'COMPLETED') return show('本场面试已完成，请生成报告或创建新计划'); if (session.status !== 'CREATED') return loadQuestion(session.id) } const s = await api<Session>('/api/v1/interviews', { method: 'POST', body: JSON.stringify({ plan_id: plan?.id ?? ids.planId }) }, requireUser()); const active = await api<Session>(`/api/v1/interviews/${s.id}/start`, { method: 'POST' }, requireUser()); setSession(active); saveIds({ ...ids, sessionId: s.id }); await loadQuestion(s.id) }
  const submit = async () => { if (!session) return; if (!pendingEvaluation) { try { await api<{ id: string }>(`/api/v1/interviews/${session.id}/answers`, { method: 'POST', body: JSON.stringify({ answer_text: answer, idempotency_key: crypto.randomUUID() }) }, requireUser()) } catch (error) { if (!(error instanceof ApiRequestError) || error.status !== 409) throw error } } const ev = await api<Evaluation>(`/api/v1/interviews/${session.id}/evaluate`, { method: 'POST' }, requireUser()); const refreshed = await api<Session>(`/api/v1/interviews/${session.id}`, {}, requireUser()); setEvaluation(ev); setSession(refreshed); if (refreshed.status === 'COMPLETED') { setQuestion(''); show(`本场面试已完成，最后一题得分 ${ev.overall_score}/100`) } else show(`已评分：${ev.overall_score}/100`) }
  ...
  return <section className="stack"><div className="card actions"><div><h2>面试控制台</h2><p>计划由 Interview Agent 生成；刷新页面后会恢复到已保存的会话。</p></div><div><button className="secondary" disabled={!ids.resumeId || !ids.jobId} onClick={() => createPlan().catch(e => show(e.message))}>生成计划</button><button className="primary" disabled={!plan && !ids.planId} onClick={() => start().catch(e => show(e.message))}>{session ? '继续面试' : '开始面试'}</button></div></div>{session?.status === 'COMPLETED' && <div className="card hero"><h2>本场面试已完成</h2><p>所有计划题与追问均已结束，现在可以生成整场复盘报告。</p><button className="primary" onClick={() => report().catch(e => show(e.message))}>{reportData ? '重新生成整场报告' : '生成整场报告'}</button></div>}{reportData && <div className="card stack"><div className="grid three"><Metric label="总体评分" value={`${reportData.summary.overall_score}/100`} /><Metric label="已评价题数" value={reportData.summary.evaluated_question_count} /><Metric label="薄弱主题" value={reportData.weak_topics.length} /></div><h2>整场面试报告</h2><h3>优势能力</h3><Tags items={reportData.summary.strong_skills} /><h3>优先复习</h3><Tags items={reportData.weak_topics} /><h3>下一次训练建议</h3><ul className="path">{reportData.recommended_actions.map(action => <li key={action}>{action}</li>)}</ul></div>}{question && <div className="card question"><span className="pill">当前问题</span><h2>{question}</h2><textarea value={answer} disabled={pendingEvaluation} onChange={e => setAnswer(e.target.value)} placeholder={pendingEvaluation ? '回答已保存，正在等待评价…' : '输入你的回答，尽量结构化表达…'} /><button className="primary" disabled={!pendingEvaluation && !answer} onClick={() => submit().catch(e => show(e.message))}>{pendingEvaluation ? '重新尝试评价' : '提交并评价'}</button></div>}{evaluation && <div className="card evaluation"><div className="score">{evaluation.overall_score}</div><div><h2>本题复盘</h2><Tags items={evaluation.strengths} /><p>{evaluation.improved_answer}</p>{session?.status !== 'COMPLETED' && <button className="secondary" onClick={() => loadQuestion().catch(e => show(e.message))}>下一题</button>}<button className="secondary" onClick={() => report().catch(e => show(e.message))}>生成报告</button></div></div>}</section>
}

function KnowledgePanel({ userId, show, requireUser }: Pick<PanelProps, 'userId' | 'show' | 'requireUser'>) {
  const [file, setFile] = useState<File | null>(null); const [results, setResults] = useState<Array<{ content: string; source_name: string; score: number }>>([]); const [query, setQuery] = useState('')
  ...
  const search = async () => { const data = await api<typeof results>('/api/v1/knowledge/search', { method: 'POST', body: JSON.stringify({ query }) }, requireUser()); setResults(data) }
  return <section className="stack"><div className="card"><h2>项目资料</h2><p>支持 PDF、DOCX、Markdown 与 TXT。所有资料只在当前用户范围内检索。</p><input type="file" accept=".pdf,.docx,.md,.txt" onChange={(e: ChangeEvent<HTMLInputElement>) => setFile(e.target.files?.[0] ?? null)} /><button className="primary" disabled={!file} onClick={() => upload().catch(e => show(e.message))}>上传项目资料</button></div><div className="card"><h2>检索你的资料</h2><div className="inline"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="例如：项目中的 RAG 评估指标" /><button className="secondary" disabled={!query} onClick={() => search().catch(e => show(e.message))}>检索</button></div>{results.map((r, i) => <article className="result" key={i}><b>{r.source_name}</b><span>{r.score.toFixed(3)}</span><p>{r.content}</p></article>)}</div></section>
}

function ProgressPanel({ userId, show }: Pick<PanelProps, 'userId' | 'show'>) {
  const [data, setData] = useState<{ weakest_topics: string[]; next_reviews: Array<{ skill_name: string; mastery_score: number; next_review_at: string }> } | null>(null)
  useEffect(() => { if (userId) api<typeof data>('/api/v1/progress/overview', {}, userId).then(setData).catch(e => show(e.message)) }, [userId])
  return <section className="grid two"><div className="card"><h2>薄弱主题</h2><Tags items={data?.weakest_topics ?? []} /></div><div className="card"><h2>复习计划</h2>{data?.next_reviews.map(item => <div className="review" key={item.skill_name}><b>{item.skill_name}</b><span>{item.mastery_score.toFixed(0)} 分 · {new Date(item.next_review_at).toLocaleDateString()}</span></div>)}</div></section>
}

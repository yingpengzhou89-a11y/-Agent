...
function InterviewPanel({ userId, ids, saveIds, show, requireUser }: PanelProps) {
  ...
  const loadQuestion = async (sessionId = session?.id) => { if (!sessionId) return; const q = await api<{ id: string; question_text: string }>(`/api/v1/interviews/${sessionId}/question`, {}, requireUser()); setQuestionId(q.id); setQuestion(q.question_text); setAnswer(''); setEvaluation(null) }
...
function KnowledgePanel({ userId, show, requireUser }: Pick<PanelProps, 'userId' | 'show' | 'requireUser'>) {
  const [file, setFile] = useState<File | null>(null); const [searchData, setSearchData] = useState<KnowledgeSearchResponse | null>(null); const [quality, setQuality] = useState<KnowledgeQuality | null>(null); const [feedback, setFeedback] = useState<Record<string, string>>({}); const [query, setQuery] = useState('')
  ...
  useEffect(() => { loadQuality().catch(() => undefined) }, [userId])
  ...
  return <section className="stack"><div className="card"><h2>项目资料</h2><p>支持 PDF、DOCX、Markdown 与 TXT。所有资料只在当前用户范围内检索。</p><input type="file" accept=".pdf,.docx,.md,.txt" onChange={(e: ChangeEvent<HTMLInputElement>) => setFile(e.target.files?.[0] ?? null)} /><button className="primary" disabled={!file} onClick={() => upload().catch(e => show(e.message))}>上传项目资料</button></div><div className="grid three"> <Metric label="检索次数" value={quality?.search_count ?? 0} /><Metric label="零结果率" value={`${Math.round((quality?.zero_result_rate ?? 0) * 100)}%`} /><Metric label="引用有帮助率" value={quality?.helpful_rate == null ? '待反馈' : `${Math.round(quality.helpful_rate * 100)}%`} /></div><div className="card"><h2>检索你的资料</h2><div className="inline"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="例如：项目中的 RAG 评估指标" /><button className="secondary" disabled={!query} onClick={() => search().catch(e => show(e.message))}>检索</button></div>{searchData && <p className="muted">召回：{String(searchData.retrieval_config.lexical_retriever)} · 向量检索：{searchData.retrieval_config.vector_retriever ? '已启用' : '未配置'} · 融合：RRF · 平均历史耗时 {Math.round(quality?.average_latency_ms ?? 0)}ms</p>}{searchData?.results.map((r, i) => <article className="result" key={r.chunk_id}><b>引用 [{i + 1}] · {r.source_name}</b><span>{r.score.toFixed(3)}</span><p>{r.content}</p><small>来源类型：{r.source_type} · 片段：{r.chunk_id.slice(0, 8)}</small><div className="inline"><button className="secondary" disabled={feedback[r.chunk_id] === 'helpful'} onClick={() => rateCitation(r.chunk_id, 'helpful').catch(e => show(e.message))}>有帮助</button><button className="secondary" disabled={feedback[r.chunk_id] === 'not_helpful'} onClick={() => rateCitation(r.chunk_id, 'not_helpful').catch(e => show(e.message))}>不相关</button></div></article>)}</div></section>
}

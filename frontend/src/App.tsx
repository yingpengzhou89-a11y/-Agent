...
function KnowledgePanel({ userId, show, requireUser }: Pick<PanelProps, 'userId' | 'show' | 'requireUser'>) {
  ...
  return <section className="stack"><div className="card"><h2>项目资料</h2><p>支持 PDF、DOCX、Markdown 与 TXT。所有资料只在当前用户范围内检索。</p><input type="file" accept=".pdf,.docx,.md,.txt" onChange={(e: ChangeEvent<HTMLInputElement>) => setFile(e.target.files?.[0] ?? null)} /><button className="primary" disabled={!file} onClick={() => upload().catch(e => show(e.message))}>上传项目资料</button><p className="muted">Embedding：{embedding?.configured ? `${embedding.model}（${embedding.dimensions} 维）` : '未配置'} · 已索引 {embedding?.indexed_document_count ?? 0} 篇 · 待建立 {embedding?.pending_document_count ?? 0} 篇</p><button className="secondary" disabled={!embedding?.configured || !((embedding?.indexed_document_count ?? 0) + (embedding?.pending_document_count ?? 0))} onClick={() => reindexAll().catch(e => show(e.message))}>重新切分并重建全部索引</button></div><div className="grid three"> <Metric label="检索次数" value={quality?.search_count ?? 0} /><Metric label="零结果率" value={`${Math.round((quality?.zero_result_rate ?? 0) * 100)}%`} /><Metric label="引用有帮助率" value={quality?.helpful_rate == null ? '待反馈' : `${Math.round(quality.helpful_rate * 100)}%`} /></div><div className="card"><h2>检索你的资料</h2><div className="inline"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="例如：项目中的 RAG 评估指标" /><button className="secondary" disabled={!query} onClick={() => search().catch(e => show(e.message))}>检索</button></div>{searchData && <p className="muted">召回：{String(searchData.retrieval_config.lexical_retriever)} · 向量检索：{searchData.retrieval_config.vector_retriever ? '已启用' : '未配置（语义相近的同义改写仍需配置 Embedding）'} · 排序：{String(searchData.retrieval_config.fusion)}{searchData.retrieval_config.feedback_personalization ? ' · 已应用你的引用反馈' : ''} · 默认展示 Top 3 · 平均历史耗时 {Math.round(quality?.average_latency_ms ?? 0)}ms</p>}{searchData?.results.map((r, i) => <article className="result" key={r.chunk_id}><b>引用 [{i + 1}] · {r.source_name}</b><span>{r.score.toFixed(3)}</span><p>{r.content}</p><small>来源类型：{r.source_type} · 片段：{r.chunk_id.slice(0, 8)}</small><div className="inline"><button className="secondary" disabled={feedback[r.chunk_id] === 'helpful'} onClick={() => rateCitation(r.chunk_id, 'helpful').catch(e => show(e.message))}>有帮助</button><button className="secondary" disabled={feedback[r.chunk_id] === 'not_helpful'} onClick={() => rateCitation(r.chunk_id, 'not_helpful').catch(e => show(e.message))}>不相关</button></div></article>)}</div></section>
}

function ProgressPanel({ userId, show }: Pick<PanelProps, 'userId' | 'show'>) {
  const [data, setData] = useState<{ completed_interviews: number; evaluated_answers: number; weakest_topics: string[]; next_reviews: Array<{ skill_name: string; mastery_score: number; next_review_at: string; consecutive_correct_count: number; consecutive_incorrect_count: number }> } | null>(null)
  const [history, setHistory] = useState<{ history: Array<{ session_id: string; overall_score: number; evaluated_question_count: number; weak_topics: string[]; completed_at: string | null }>; score_change: number | null; recent_average_score: number | null } | null>(null)
  useEffect(() => { if (userId) { api<typeof data>('/api/v1/progress/overview', {}, userId).then(setData).catch(e => show(e.message)); api<typeof history>('/api/v1/progress/history', {}, userId).then(setHistory).catch(e => show(e.message)) } }, [userId])
  return <section className="stack"><div className="grid three"><Metric label="完成面试" value={data?.completed_interviews ?? 0} /><Metric label="已评价回答" value={data?.evaluated_answers ?? 0} /><Metric label="近 3 场均分" value={history?.recent_average_score == null ? '暂无' : `${history.recent_average_score}/100`} /></div><div className="grid two"><div className="card"><h2>薄弱主题</h2><Tags items={data?.weakest_topics ?? []} /></div><div className="card"><h2>动态复习计划</h2>{data?.next_reviews.map(item => <div className="review" key={item.skill_name}><b>{item.skill_name}</b><span>{item.mastery_score.toFixed(0)} 分 · {new Date(item.next_review_at).toLocaleDateString()} · 连续答对 {item.consecutive_correct_count} 次 / 答错 {item.consecutive_incorrect_count} 次</span></div>)}</div></div><div className="card"><h2>面试历史与得分趋势</h2><p className="muted">{history?.score_change == null ? '完成两场面试后，将展示相对上一场的得分变化。' : `最近一场较上一场 ${history.score_change >= 0 ? '提升' : '下降'} ${Math.abs(history.score_change)} 分。`}</p>{history?.history.length ? <div className="path">{history.history.map((item, index) => <div className="review" key={item.session_id}><b>第 {history.history.length - index} 场 · {item.overall_score}/100</b><span>{item.evaluated_question_count} 题已评价 · {new Date(item.completed_at ?? '').toLocaleDateString()} · 薄弱项：{item.weak_topics.join('、') || '暂无'}</span></div>)}</div> : <p className="muted">完成面试并生成整场报告后，历史记录会显示在这里。</p>}</div></section>
}
